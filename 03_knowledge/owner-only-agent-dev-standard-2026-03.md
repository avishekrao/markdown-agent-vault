...
- Agent:- performs decomposition, implementation, testing and assembly;
...
- The agent is prohibited without explicit confirmation from the user:
  ...
  - data migration without a confirmed rollback path.### 5) Self-test circuit (mandatory)
...
If the agent is running in a loop and these mechanisms are not present, the `GREEN` status is disabled even if tests are passing.

### 6) Format of communication with the owner (traffic light only)The agent returns only one of the statuses:
